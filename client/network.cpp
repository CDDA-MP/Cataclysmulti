...
namespace Network
{
    ...
    static void sendRaw(const char* data, unsigned long len)      // Send raw data to server
    {
        if (Network::connected)
        {
            ...
            buf.base[len] = ',';
            uv_write(wr, (uv_stream_t*) &Network::socket, &buf, 1, [](uv_write_s*wr, int)
            {
                delete wr;
            });
        }
    ...
    void connect(const char* addr, int port)
    {
        ...
        uv_tcp_connect(&req, &socket, &dest, [](uv_connect_t* req, int status)
        {
            if (status == 0)
            {
    ...
    void disconnect()
    {
        if (Network::connected)
        {
            ...
            {
                uv_close((uv_handle_s*) &Network::socket, [](uv_handle_s*)
                {
                    Network::connected = false;
                });
            }
